...
describe('Install and Run (e2e)', () => {
  ...
  it('installed package contains dist/src/index.js', () => {
    const indexPath = join(tempDir, 'node_modules/@calabamatex/agentsentry/dist/src/index.js');
    expect(existsSync(indexPath)).toBe(true);
  });

  it('installed package contains config resolution module', () => {
    const resolvePath = join(tempDir, 'node_modules/@calabamatex/agentsentry/dist/src/config/resolve.js');
    expect(existsSync(resolvePath)).toBe(true);
  });

  it('installed package contains agent-sentry.config.json', () => {
    const configPath = join(tempDir, 'node_modules/@calabamatex/agentsentry/agent-sentry.config.json');
    expect(existsSync(configPath)).toBe(true);
  });

  it('CLI entry point exists and is valid JS', () => {
    const cliPath = join(tempDir, 'node_modules/@calabamatex/agentsentry/dist/src/cli/index.js');
  ...
  it('MCP server entry point exists and is valid JS', () => {
    const mcpPath = join(tempDir, 'node_modules/@calabamatex/agentsentry/dist/src/mcp/server.js');
