...
# Parse config
CLAUDE_MD_MAX_LINES=$(jq -r '.rules_file.claude_md_max_lines // 300' "$CONFIG_FILE" 2>/dev/null || echo 300)
AGENTS_MD_MAX_LINES=$(jq -r '.rules_file.agents_md_max_lines // 150' "$CONFIG_FILE" 2>/dev/null || echo 150)
