...
- Optional Supabase integration CI job (WI-012): runs the credential-gated integration + smoke tests when `SUPABASE_TEST_URL`/`SUPABASE_TEST_KEY` secrets are set, and skips cleanly (exit 0) otherwise so forks stay green. Setup documented in `docs/supabase-setup.md`.
